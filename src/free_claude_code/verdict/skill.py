...
Installation is explicit (only runs on ``fcc-verdict install-claude-skill``),
...
name: deep-verdict
description: >-
  Consult FCC Verdict — several free cloud models that propose, cross-review,
...
Use FCC Verdict when the user asks to:
...
```bash
fcc-verdict evaluate --output json --depth deep "<question>"
```
...
or the `fcc-verdict` CLI core only.
"""
