"""``fcc-trace`` — explain what happened in a proxy turn from ``server.log``.

Examples
--------
    fcc-trace                 # slowest recent turns
    fcc-trace --last          # the most recent turn
    fcc-trace req_abc123      # one turn by id
    fcc-trace --slow 20       # turns longer than 20s
    fcc-trace --json          # machine-readable
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="path to server.log (default: ~/.fcc/logs/server.log)",
    )
