...
from llmux.cli.launchers.common import preflight_proxy
from llmux.cli.process_registry import (
...
from llmux.config.paths import (
    config_dir_path,
    legacy_env_paths,
    managed_env_path,
)
from llmux.config.server_urls import local_admin_url, local_proxy_root_url
...
def serve(argv: Sequence[str] | None = None) -> None:
    """Start the FastAPI server (registered as `llmux-server` script)."""
...
def _schedule_open_admin_browser(settings: Settings) -> None:
    ...
    threading.Thread(
        target=open_when_ready, name="llmux-open-admin-browser", daemon=True
    ).start()
...
def init(argv: Sequence[str] | None = None) -> None:
    """Scaffold config at ~/.llmux/.env (registered as `llmux-init`)."""
    if _print_version_if_requested(argv):
        return
    ...
    if migrated_from is not None:
        ...
        print(
            "Edit it to set your API keys and model preferences, then run: llmux-server"
        )
        return
    ...
    print("Edit it to set your API keys and model preferences, then run: llmux-server")
...
def _migrate_legacy_env_if_missing() -> Path | None:
    ...
    # TODO: Remove after the ~/.llmux/.env migration has had a release cycle.
