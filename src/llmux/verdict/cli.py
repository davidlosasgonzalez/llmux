"""``llmux-verdict`` command-line interface.

Default output is Markdown; ``--output json`` returns the full structured
payload. The CLI is a thin shell over :class:`VerdictService` so it shares the
...
from .errors import InsufficientFreeModelsError, VerdictError
from .models import ModelRef, VerdictResult
from .service import ProviderUsage, ProviderValidation, VerdictService
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="llmux-verdict",
        description="Free-only multi-model deliberation verdict for LLMux.",
    )
    ...
    evaluate = sub.add_parser("evaluate", help="Run a verdict deliberation.")
    ...
    sub.add_parser(
        "install-claude-skill", help="Install the deep-verdict Claude Code skill."
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.version:
        print(f"llmux-verdict {package_version()}")
        return
...
async def _run_benchmark(service: VerdictService, output: str) -> None:
...
def _install_skill() -> None:
    ...
    print("Installed the deep-verdict skill:")
    for path in created:
        print(f"  - {path}")
...
def _render_result(result: VerdictResult, report_path: object) -> str:
...
def _fail_insufficient(exc: InsufficientFreeModelsError) -> None:
    print(f"Verdict could not run: {exc}", file=sys.stderr)
    if exc.reasons:
