"""Installer for the ``deep-verdict`` Claude Code skill and MCP registration.

Installation is explicit (only runs on ``llmux-verdict install-claude-skill``),
...
SKILL_NAME = "deep-verdict"

SKILL_MD = """\
---
name: deep-verdict
description: >-
  Consult LLMux Verdict — several free cloud models that propose, cross-review,
...
Use LLMux Verdict when the user asks to:
...
```bash
llmux-verdict evaluate --output json --depth deep "<question>"
```
...
or the `llmux-verdict` CLI core only.
"""
