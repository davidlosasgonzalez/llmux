...
from llmux.core.anthropic import anthropic_error_type_for_failure
...
from llmux.core.async_iterators import try_close_async_iterator
from llmux.core.diagnostics import safe_exception_message
from llmux.core.failures import find_execution_failure
from llmux.core.trace import close_stream_input, trace_event
    ...
    async def _close(self, *, preserved_error: BaseException | None) -> None:
        ...
        if task is None:
            task = asyncio.create_task(
                ...
                name="llmux-api-response-cleanup",
            )
...
def _trace_response_cleanup_failure(operation: str, exc: BaseException) -> None:
    trace_event(
        stage="egress",
        event="llmux.api.response.cleanup_failed",
        source="api",
        operation=operation,
...
async def _close_pre_start_body(
    ...
    *,
    ...
    task = asyncio.create_task(
        ...
        name="llmux-api-pre-start-stream-cleanup",
    )
    await _wait_for_cleanup(task)
