    ...
    def stream(
        ...
        *,
        ...
        return traced_async_stream(
            ...
            source="api",
            complete_event=(
                "llmux.api.responses.stream_completed"
                if wire_api == "responses"
                else "llmux.api.response.stream_completed"
            ),
            interrupted_event=(
                "llmux.api.responses.stream_interrupted"
                if wire_api == "responses"
                else "llmux.api.response.stream_interrupted"
            ),
            chunk_event=None,
            extra=stream_trace,
        )
    ...
    def _trace_ingress(
        ...
        *,
        ...
        trace_event(
            stage="ingress",
            event=(
                "llmux.api.responses.request.received"
                if wire_api == "responses"
                else "llmux.api.request.received"
            ),
            source="api",
