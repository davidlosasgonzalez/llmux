...
- Con logging apuntando a `~/.fcc/logs/council-mcp.log` (mismo sink que
  `fcc-council serve-mcp`) y `GROQ_API_KEY` inválida, un invoke real escribió:
  - `ERROR: GROQ_ERROR: ... AuthenticationError http_status=401`
  - `WARNING: council.provider.error model=groq/... Invalid API Key`

### Prueba B — Cloudflare Workers CPU limits

- `fcc-council evaluate --depth quick --research on` con Brave.
...
- **Causa raíz:** `Settings` (config/settings.py) no tenía un campo
  `brave_search_api_key`; `CouncilService._research_service()` llamaba a
