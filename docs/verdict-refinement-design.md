...
infrastructure is reused; this is an **additive layer** (`verdict/` package),
...
first verdict's answer back as the query of a *fresh* verdict run. Opt-in, roughly
...
- [x] **Capability priors (cold-start fix).** `verdict/capability.py` derives a
...
- [x] **Budget-class routing.** `verdict/provider_limits.py` classifies each
...
- **Same core** as the CLI (`VerdictService`), so behaviour is identical whether
  invoked by MCP, CLI or tests.
- The `deep-verdict` skill tells Claude *when* to reach for it (second opinion,
...
shipped — see `verdict.md`. The remaining unchecked items above (role-capability
