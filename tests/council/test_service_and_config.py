...
from free_claude_code.council.models import Privacy
from free_claude_code.council.research import ResearchService, SearchHit
from free_claude_code.council.service import CouncilService
from tests.council.support import FakeInvoker, make_model
...
def _three_provider_service(monkeypatch, *, research_service=None, invoker=None):
    ...
    config = CouncilConfig(enabled_providers=["groq", "cerebras", "gemini"])
    lister = _lister(
        {
...
async def test_full_report_carries_timing(monkeypatch):
    from free_claude_code.council.service import _full_report
...
# --------------------------------------------------------------------------- #
def _ctx_service(monkeypatch) -> CouncilService:
    settings = _settings_with(monkeypatch, GROQ_API_KEY="k1")
    return _service(settings, CouncilConfig(), _lister({}))
