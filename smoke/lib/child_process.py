...
(``llmux.exe`` in use), causing flaky smoke. The smoke runner is
...
def cmd_llmux_init() -> list[str]:
    return [
        python_exe(),
        "-c",
        "from llmux.cli.entrypoints import init; init()",
    ]


def cmd_llmux_version() -> list[str]:
    return [
        python_exe(),
        "-c",
        (
            "import sys; "
            "sys.argv = ['llmux-server', '--version']; "
            "from llmux.cli.entrypoints import serve; serve()"
        ),
    ]


def cmd_llmux_serve() -> list[str]:
    return [
        python_exe(),
        "-c",
        "from llmux.cli.entrypoints import serve; serve()",
